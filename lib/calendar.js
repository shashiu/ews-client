...
Calendar.prototype.getItemDetails = function (itemId, changeKey) {
    var ewsSoapReq = this._session.createEwsRequest(
            '<soap:Body><m:GetItem>'+
                '<m:ItemShape><t:BaseShape>AllProperties</t:BaseShape>' +
                   '<t:AdditionalProperties><t:ExtendedFieldURI DistinguishedPropertySetId="PublicStrings" '+
                        'PropertyName="OnlineMeetingExternalLink" PropertyType="String" /></t:AdditionalProperties>' +
                '</m:ItemShape>'+
                ...
                var entries = xmlObj['s:Envelope']['s:Body'][0]['m:GetItemResponse'][0]
                    ...
                    .map(function (obj) {
                        ...
                        var item = CalendarItem(
                                        obj['t:ItemId'][0]['Id'],
                                        obj['t:ItemId'][0]['ChangeKey']
                                        );
                        item.Subject = ['t:Subject'][0];
                        item.Start = obj['t:Start'][0];
                        item.End = obj['t:End'][0];
...
Calendar.prototype.getEntries = function (startTime, endTime) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            xml2js.parseString(response, function (err, xmlObj) {
                var items = xmlObj['s:Envelope']['s:Body'][0]['m:FindItemResponse'][0]
                                  ['m:ResponseMessages'][0]['m:FindItemResponseMessage'][0]
                                  ['m:RootFolder'][0]['t:Items'][0]['t:CalendarItem'];
                if (items != null) {
                    entries = items.map(function (obj) {
                        ...
                        var entry = new CalendarItem(obj['t:ItemId'][0]['Id'], obj['t:ItemId'][0]['ChangeKey']);
                        entry.Subject = obj['t:Subject'][0];
                        entry.Start = obj['t:Start'][0];
                        entry.End = obj['t:End'][0];
                    ...
                    entries = calendarEvents[0]['CalendarEvent']
                        .map(function(obj) {
                            var item = new CalendarItem(
                                ...
                                obj['CalendarEventDetails'][0]['ChangeKey'][0]
                                );
                            item.Subject = obj['CalendarEventDetails'][0]['Subject'][0];
                            item.Start = obj['CalendarEventDetails'][0]['StartTime'][0];
                            item.End = obj['CalendarEventDetails'][0]['EndTime'][0];
                            return item;
                        });
                }
                resolve(entries); 
