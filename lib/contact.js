    ...
    return this._session.makeEwsRequest(soapReq).then(function (response) {
        return new Promise(function (resolve, reject) {
            ...
            xml2js.parseString(response, function (err, xmlObj) {
                var contactxml = xmlObj['s:Envelope']['s:Body'][0]['m:ResolveNamesResponse'][0]
                                    ['m:ResponseMessages'][0]['m:ResolveNamesResponseMessage'][0]
                                    ['m:ResolutionSet'][0]['t:Resolution'][0]['t:Contact'][0];
                var emailAddress = xmlObj['s:Envelope']['s:Body'][0]
                                    ['m:ResolveNamesResponse'][0]['m:ResponseMessages'][0]
                                    ['m:ResolveNamesResponseMessage'][0]['m:ResolutionSet'][0]
                                    ['t:Resolution'][0]['t:Mailbox'][0]['t:EmailAddress'][0];
                var contact = null;
                if (contactxml != null) {
                    var i;
                    var phoneNumbers = [];
        ...
        return new Promise(function (resolve, reject) {
            xml2js.parseString(response, function (err, xmlObj) {
                var roomLists = xmlObj['s:Envelope']['s:Body'][0]
                                      ['GetRoomListsResponse'][0]
                                      ['m:RoomLists'][0]['t:Address'];
                var addresses = null;
                if (roomLists != null) {
                    addresses = roomLists
                        .map(function (obj) {
        ...
        return new Promise(function (resolve, reject) {
            xml2js.parseString(response, function (err, xmlObj) {
                var rooms = xmlObj['s:Envelope']['s:Body'][0]
                                  ['m:ExpandDLResponse'][0]['m:ResponseMessages'][0]
                                  ['m:ExpandDLResponseMessage'][0]['m:DLExpansion'];
